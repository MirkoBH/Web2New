...
if (!sesion) {
  ...
  mount.innerHTML = `
    <section class="glass-panel p-4 mb-4">
      ...
      <p class="text-secondary mb-0">Bienvenido/a, <strong>${sesion.nombre}</strong>.</p>
    </section>
    ...
    <!-- Consultas recibidas en mis publicaciones -->
    <section class="glass-panel p-4 mb-4">
      <h2 class="h4 mb-3">Consultas en mis publicaciones</h2>
  ...
  if (!wishlistIds.length) {
    gridFav.innerHTML = `<p class="text-secondary">Aún no guardaste ningún auto. <a href="listado.html" class="text-danger">Explorá el listado.</a></p>`;
  } else {
    Promise.all(wishlistIds.map((id) => carsApi.obtener(id).catch(() => null)))
      .then((autos) => {
        const validos = autos.filter(Boolean).map(mapearAuto);
        if (!validos.length) { gridFav.innerHTML = `<p class="text-secondary">No se encontraron los favoritos.</p>`; return; }
